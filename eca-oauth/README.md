...
2) spring.boot.admin.client - настройки клиента для сервиса мониторинга
   ...
   * instance.metadata.user - credentials пользователя для передачи на сервис мониторинга. С помощью этих
     данных сервис мониторинга получает доступ к конечным точкам /actuator/** клиентского приложения, используя
     http basic авторизацию.
   * url - url сервиса мониторинга
   * username - логин пользователя для авторизации в сервисе мониторинга
   * password - пароль пользовтаеля для авторизации в сервисе мониторинга
