...
audit:
  ...
  ## Начальное время ожидания в сек. перед первым запуском заданий на повторную отправку событий аудита
  redeliveryInitialDelaySeconds: 120
...
Примечание: Также можно использовать любую другу реализация LockRegistry от spring integration.
