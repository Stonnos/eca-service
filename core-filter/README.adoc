...
----
filter:
  ## Путь к шаблонам фильтров (опционально)
  templatesLocation: classpath*:filter-templates/templates/**/*.json
  ## Путь к шаблонам справочников для фильтров (опционально)
  dictionariesLocation: classpath*:filter-templates/dictionaries/**/*.json
----
...
Справочники необходимо выносить в отдельные json конфигурации, когда они встречаются в нескольких шаблонах.
В целом можно задавать и в самом конфиге шаблона фильтра (поле *field.dictionary*)
