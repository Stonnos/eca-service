...
|instances_object_id     |bigint           |-|+|fk_instances_instances_object_id            |ID объекта обучающей выборки
|===

==== Таблица 3.{counter:db-table-id} - 'instances_object' - содержит данные об обучающих выборках
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                           |Идентификатор записи (первичный ключ)
|md5_hash                |varchar(255)     |-|+|instances_info_data_md5_hash_unique_idx     |MD5 хеш данных
