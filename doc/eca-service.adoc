...
|Модуль для отправки уведомлений (email, web push)
|12
|Eca audit log
...
image::./images/eca-ss-architecture.png[alt=Диаграмма взаимодействия с системными сервисами,scaledwidth=80%]
...
|9
|Filebeat
|Система для чтения логов docker - контейнеров с последующей их отправкой в victorialogs хранилище
|10
|Victorialogs
...
|auc_predictions_path       |varchar(255)     |-|-|-                                    |Путь к объекту в S3 с данными для ROC кривых
|===
...
==== Таблица 3.{counter:db-table-id} - 'evaluation_results_attachment' - таблица содержит данные с файлами результатов классификации
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                           |bigint           |+|+|+                         |Идентификатор записи (первичный ключ)
|attachment_key               |varchar(255)     |+|+|+                         |Ключ файла
|attachment_type              |varchar(255)     |+|+|+                         |Тип файла
|file_path                    |varchar(255)     |-|+|-                         |Путь к файлу
|created_date                 |timestamp        |-|+|-                         |Дата создания
|updated_date                 |timestamp        |-|+|-                         |Дата обновления
|===

Таблица содержит уникальный индекс *evaluation_results_attachment_key_attachment_type_unique_idx* на поля *attachment_key, attachment_type*.
...
|old_email               |varchar(255)     |-|+|-                                                   |Старый Email
|user_id                 |bigint           |-|+|fk_change_email_request_user_id                     |Внешний ключ пользователя
|created                 |timestamp        |-|+|-                                                   |Дата создания записи
|revocation_token        |varchar(255)     |-|+|change_email_request_revocation_token_unique_index  |Токен для отмены изменения email
|revocation_expire_at    |timestamp        |-|+|-                                                   |Дата истечения срока действия токена для отмены изменения email
|revocation_date         |timestamp        |-|-|-                                                   |Дата отмены изменения email
|===
...
==== Таблица 3.{counter:db-table-id} - 'force_set_password_request' - содержит данные о запросах на установку постоянного пароля при первом входе
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                              |Идентификатор записи (первичный ключ)
|token                   |varchar(255)     |+|+|force_set_password_request_token_unique_index  |Короткоживущий токен для установки пароля
|expire_date             |timestamp        |-|+|-                                              |Дата истечения срока действия токена
|password_date           |timestamp        |-|-|-                                              |Дата установки пароля
|user_id                 |bigint           |-|+|fk_reset_password_request_user_id              |Внешний ключ пользователя
|confirmation_code       |varchar(255)     |-|+|-                                              |Код подтверждения из письма для установки пароля
|created                 |timestamp        |-|+|-                                              |Дата создания записи
|===

==== Таблица 3.{counter:db-table-id} - 'personal_access_token' - содержит данные токенов доступа к message api для построения моделей
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                          |Идентификатор записи (первичный ключ)
|token                   |varchar(255)     |+|+|personal_access_token_unique_index         |Md5 хеш токена
|token_name              |varchar(255)     |+|+|+                                          |Уникальное имя токена
|token_type              |varchar(255)     |-|+|+                                          |Тип токена (USER_TOKEN - пользовательский, APP_TOKEN - токен приложения)
|expire_date             |timestamp        |-|+|-                                          |Дата истечения срока действия токена
|user_id                 |bigint           |-|+|fk_personal_access_token_user_id           |Внешний ключ пользователя
|created                 |timestamp        |-|+|-                                          |Дата создания записи
|===

Таблица также содержит уникальный индекс *add_personal_access_token_token_name_user_id_unique_index* на поля token_name, user_id
...
|last_exported_date      |timestamp        |-|+|-                                      |Дата последней выгрузки датасета для классификации в единое хранлище данных
|===
...
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                        |Идентификатор записи (первичный ключ)
|external_data_uuid      |varchar(255)     |+|+|-                                        |UUID обучающей выборки из единого хранилища данных
...
|instances_id            |bigint           |-|+|fk_export_instances_object_instances_id  |ID обучающей выборки
|===

Таблицы *databasechangeloglock* и *databasechangeloglock* предназначены для управления миграциями базы данных.

=== 3.9 Схема БД eca-audit-log

На рисунке 3.5 приведена схема базы данных eca-audit-log:

image::./images/eca-audit-log-db-schema.png[alt=Схема базы данных eca-audit-log,scaledwidth=80%]

=== 3.10 Описание таблиц БД eca-audit-log
...
=== 3.11 Схема БД eca-notification-service

На рисунке 3.6 приведена схема базы данных eca-notification-service:

image::./images/eca-notifications-db-schema.png[alt=Схема базы данных eca-notification-service,scaledwidth=80%]

=== 3.12 Описание таблиц БД eca-notification-service
...
==== Таблица 3.{counter:db-table-id} - 'user_notification' - содержит данные об уведомлениях пользователей
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint            |+|+|+                               |Идентификатор записи (первичный ключ)
...
==== Таблица 3.{counter:db-table-id} - 'notification_options' - содержит данные о настройках уведомлений для пользователей
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                     |Идентификатор записи (первичный ключ)
...
|user                    |мфксрфк(255)     |-|+|notification_options_user_unique_idx  |Логин пользователя
|===

==== Таблица 3.{counter:db-table-id} - 'user_notification_event_options' - содержит данные о настройках событий уведомлений для пользователей
[cols="^20%,^14%,^8%,^8%,^8%,^30%",options="header"]
|===
|Название колонки|Тип|Unique|Not NULL|Индекс|Описание
|id                      |bigint           |+|+|+                                                           |Идентификатор записи (первичный ключ)
...
|notification_options_id |bigint           |-|+|fk_notification_event_options_notification_options_id       |Внешний ключ notification_options
|===

Таблица также содержит уникальный индекс *notification_options_id_event_type_unique_idx* на поля notification_options_id, event_type
...
=== 3.13 Схема БД eca-data-loader

На рисунке 3.7 приведена схема базы данных eca-data-loader:

image::./images/data-loader-db-schema.png[alt=Схема базы данных eca-data-loader,scaledwidth=80%]

=== 3.14 Описание таблиц БД eca-data-loader
...
=== 3.19 Описание служебных таблиц transactional-outbox
